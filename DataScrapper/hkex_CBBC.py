...
def get_CBBC_zip_from_hkex(url, save_name, chunk_size=128):
...
def get_bulk__zip_from_hkex(start_month, end_month, combine): # include combine to excel function
    ...
    for month in datelist:
        ...
        get_CBBC_zip_from_hkex(f'https://www.hkex.com.hk/eng/cbbc/download/CBBC{month[-2:]}.zip', f'CBBC_{month}.zip',
                               chunk_size=128)
    if combine == "Y":
...
def request_daily_CBBC_rawdata(file_name):
...
if __name__ == "__main__":
    get_bulk__zip_from_hkex("2021-01-01", "2021-08-01", "Y")
